...
class Requestable {
   ...
   __getRequestHeaders() {
      let headers = {
         ...
         'Accept': (AcceptHeader || this.__AcceptHeader),
      };
      return Object.assign(headers, this.__authorizationHeader);
   }

    /**
    * Compute the headers required for an API request.
    * @private
    * @param {object} AuthHeader - the auth header for the request
    * @return {Object} - the headers to use in the request
    */
    __setAHeaders(auth) {
   ...
   _request(method, path, data, options, cb) {
      ...
      if (options && options.headers) {
         this.__setHeaders(options.headers);
      }

      const headers = this.__getRequestHeaders();
